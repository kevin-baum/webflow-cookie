...
(function() {
    ...
    const injectCSS = () => {
        ...
        style.textContent = `
            ...
            .scanning-progress{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:16px;border-radius:8px;margin:12px 0;text-align:center;animation:pulse 2s ease-in-out infinite}
            .scanning-progress .spinner{display:inline-block;width:20px;height:20px;border:3px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin 1s linear infinite;margin-right:10px;vertical-align:middle}
            @keyframes spin{to{transform:rotate(360deg)}}
            @keyframes pulse{0%,100%{opacity:1}50%{opacity:.85}}
            .scan-complete{background:#48bb78;color:#fff;padding:12px;border-radius:6px;margin:12px 0;text-align:center;animation:slideIn .3s ease-out}
            @keyframes slideIn{from{transform:translateY(-10px);opacity:0}to{transform:translateY(0);opacity:1}}
            .scan-status{font-size:14px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px}
        `;
    ...
    CookieScanner.prototype.scan = function() {
        ...
        // Mark scan as complete and update UI
        this.scanning = false;
    ...
    CookieScanner.prototype.getToolsHTML = function() {
        if (this.scanning) {
            return '<div class="scanning-progress"><div class="scan-status"><span class="spinner"></span><span>Scanne Cookies und Tools...</span></div><div style="font-size:12px;margin-top:8px;opacity:0.9">Bitte warten, während wir alle Cookies auf dieser Seite analysieren</div></div>';
        }
        if (this.scanComplete && this.tools.size === 0) {
            return '<div class="scan-complete"><div class="scan-status">' + ICONS.check + ' <span>Scan abgeschlossen - Keine externen Tools erkannt</span></div></div>';
        }
        if (this.scanComplete && this.tools.size > 0) {
            var arr = Array.from(this.tools).sort();
            var html = '<div class="scan-complete" style="background:#48bb78"><div class="scan-status">' + ICONS.check + ' <strong>Scan abgeschlossen - ' + arr.length + ' Tool(s) erkannt</strong></div></div>';
            html += '<div class="scan-info"><div style="display:flex;flex-wrap:wrap;gap:8px;width:100%">';
    ...
    const init = () => {
        ...
        loadFeatherIcons(() => {
            ...
            loadCookieConsent(() => {
                ...
                // WICHTIG: Banner sofort anzeigen, dann Cookies scannen!
                console.log('🔍 Initialisiere Cookie-Banner...');
                
                // Sofortigen Scan starten (wird asynchron ausgeführt)
                setTimeout(function() {
                    console.log('🔍 Starte initialen Scan...');
                    scanner.scan();
                }, 100);
                
                // Deep Scan nach 500ms (für langsam ladende Scripts)
                setTimeout(function() {
